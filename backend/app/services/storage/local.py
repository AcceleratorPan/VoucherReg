...
class LocalStorageService:
    def __init__(self, root: Path, max_upload_mb: int = 15) -> None:
        self.root = root
        self.max_upload_bytes = max_upload_mb * 1024 * 1024
        self.root.mkdir(parents=True, exist_ok=True)

    async def save_upload(self, user_id: str, task_id: str, page_index: int, upload_file: UploadFile) -> StoredFile:
        ext = Path(upload_file.filename or "upload.bin").suffix.lower() or ".bin"
        safe_user_id = self._safe_user_id(user_id)
        relative_path = f"{safe_user_id}/tasks/{task_id}/pages/{page_index}{ext}"
        ...
        return StoredFile(path=relative_path, url=self._as_url(relative_path), width=width, height=height)
    ...
    @staticmethod
    def _detect_image_size(data: bytes) -> tuple[int, int]:
        try:
